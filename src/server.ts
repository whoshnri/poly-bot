...
import { handleTelegramWebhook, syncTelegramCommands } from "./telegram";
...
/**
 * Syncs Telegram command metadata with BotFather command list.
 */
async function syncTelegramBotCommands(c: Context) {
  // if (!verifyWakeToken(c)) {
  //   return c.json({ error: "Unauthorized." }, 401);
  // }
  return syncTelegramCommands(c);
}
 ...
 * Register this URL via Telegram setWebhook:
 * https://api.telegram.org/bot<token>/setWebhook
 */
app.post("/telegram/webhook", handleTelegramWebhook);
/**
 * One-time/manual command sync endpoint.
 * Use this after editing TELEGRAM_COMMANDS in src/telegram.ts.
 */
app.post("/telegram/commands/sync", syncTelegramBotCommands);
