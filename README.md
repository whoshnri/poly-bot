...
Webhook support is available at `POST /telegram/webhook` and is implemented with `grammy`.
...
- `TELEGRAM_WEBHOOK_SECRET` (validated against `X-Telegram-Bot-Api-Secret-Token`)
- `WAKE_API_TOKEN` (required for `POST /telegram/commands/sync`)
...
1. Register commands in Telegram: call `POST /telegram/commands/sync` with `Authorization: Bearer <WAKE_API_TOKEN>` when that token is configured.
